...
async def get_extra_pe_bot_response(url):
    try:
        ...
        async with userbot_client.conversation(bot, timeout=10) as conv:
...
async def get_short_url(long_url):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://www.amazon.in/associates/sitestripe/getShortUrl?longUrl={encoded_url}&marketplaceId=44571') as response:
...
async def process_message(event):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            for url in urls:
                try:
                    # Fetch URL to check if it's an Amazon URL
                    async with session.get(url, allow_redirects=True) as response:
                        final_url = str(response.url)

                        if is_amazon_url(final_url):
                            short_url = await get_short_url(final_url)
                            if short_url:
                                url_replacements[url] = f'<a href="{short_url}">🛒 Buy Now</a>'
                        else:
                            extra_pe_bot_response = await get_extra_pe_bot_response(final_url)
                            if extra_pe_bot_response:
                                url_replacements[url] = f'<a href="{extra_pe_bot_response}">🛒 Buy Now </a>'
                            else:
                                url_replacements[url] = url
                except Exception as e:
                    logger.error(f"Error while processing URL {url}: {e}")
                    url_replacements[url] = url

        # Replace URLs in message
        for original_url, replacement in url_replacements.items():
            message = message.replace(original_url, replacement)

        bold_message = f'<b>{message}</b>'
        elapsed_time = time.time() - start_time  # Calculate elapsed time
        await processing_message.edit(f'{bold_message}\n\nTotal execution time: {elapsed_time:.2f} seconds', parse_mode='html')
